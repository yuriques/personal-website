import { Link } from "react-router-dom";

interface HomeInfoProps {
  currentStage: number;
}

const InfoBox = ({
  text,
  link,
  btnText,
}: {
  text: string;
  link: string;
  btnText: string;
}) => (
  <div className="info-box">
    <p className="font-medium sm:text-xl text-left leading-relaxed">{text}</p>
    <Link to={link} className="neo-btn">
      {btnText}
      <svg
        xmlns="http://www.w3.org/2000/svg"
        className="h-5 w-5"
        viewBox="0 0 20 20"
        fill="currentColor"
      >
        <path
          fillRule="evenodd"
          d="M10.293 3.293a1 1 0 011.414 0l6 6a1 1 0 010 1.414l-6 6a1 1 0 01-1.414-1.414L14.586 11H3a1 1 0 110-2h11.586l-4.293-4.293a1 1 0 010-1.414z"
          clipRule="evenodd"
        />
      </svg>
    </Link>
  </div>
);

const renderContent = {
  0: (
    <h1
      className="sm:text-xl sm:leading-snug text-center py-4 px-8 text-white mx-5 rounded-2xl"
      style={{
        background: "rgba(255,255,255,0.08)",
        backdropFilter: "blur(12px)",
        WebkitBackdropFilter: "blur(12px)",
        border: "1px solid rgba(255,255,255,0.15)",
        boxShadow: "0 8px 32px rgba(0,0,0,0.2)",
      }}
    >
      Hi, I am <span className="font-semibold">Elizabeth</span>👋
      <br />A Software Engineer from Australia
    </h1>
  ),
  1: (
    <InfoBox
      text="Worked with many companies and picked up many skills along the way"
      link="/about"
      btnText="Learn more"
    />
  ),
  2: (
    <InfoBox
      text="Tinkered with many projects over the years. Check out what I've done"
      link="/projects"
      btnText="Visit my projects"
    />
  ),
  3: (
    <InfoBox
      text="Want to chat? I'm just a few keystrokes away"
      link="/contact"
      btnText="Let's talk"
    />
  ),
};

const HomeInfo = ({ currentStage }: HomeInfoProps) => {
  return renderContent[currentStage] || null;
};

export default HomeInfo;
